...
def send_verification_email(user_id, username, email):
    ...
    if SENDGRID_API_KEY != 'YOUR_SENDGRID_API_KEY':
        # SendGridを使用してメールを送信
        return send_email(email, "TeAI メールアドレスの確認", html_content)
    else:
        # APIキーが設定されていない場合はファイルに保存（開発用）
...
def send_password_reset_email(user_id, username, email):
    ...
    if SENDGRID_API_KEY != 'YOUR_SENDGRID_API_KEY':
        # SendGridを使用してメールを送信
        return send_email(email, "TeAI パスワードリセット", html_content)
    else:
        # APIキーが設定されていない場合はファイルに保存（開発用）
        reset_dir = "/var/www/teai.io/password-reset"
        os.makedirs(reset_dir, exist_ok=True)
        
        try:
            # メール内容をファイルに保存
            reset_file = os.path.join(reset_dir, f"{user_id}.html")
            with open(reset_file, "w") as f:
                f.write(html_content)
            
            # リセットURLをログに出力
            logger.info(f"Password reset URL for {username}: {reset_url}")
            return True
        except Exception as e:
            logger.error(f"Failed to save password reset email: {e}")
            return False
