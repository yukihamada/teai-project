...
TeAI 可以在交互式命令行模式下运行,允许用户通过命令行启动交互式会话。

这种模式不同于[无头模式](headless-mode),后者是非交互式的,更适合脚本编写。

## 使用 Python

要通过命令行启动交互式 TeAI 会话,请按照以下步骤操作:

1. 确保你已按照[开发设置说明](https://github.com/All-Hands-AI/TeAI/blob/main/Development.md)进行操作。
...
该命令将启动一个交互式会话,你可以在其中输入任务并接收来自 TeAI 的响应。

你需要确保通过环境变量[或 `config.toml` 文件](https://github.com/All-Hands-AI/TeAI/blob/main/config.template.toml)设置你的模型、API 密钥和其他设置。


## 使用 Docker

要在 Docker 中以命令行模式运行 TeAI,请按照以下步骤操作:

1. 将 `WORKSPACE_BASE` 设置为你希望 TeAI 编辑的目录:
...
该命令将在 Docker 中启动一个交互式会话,你可以在其中输入任务并接收来自 TeAI 的响应。
