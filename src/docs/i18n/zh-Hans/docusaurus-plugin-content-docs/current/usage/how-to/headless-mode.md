...
你可以使用单个命令运行 TeAI,而无需启动 Web 应用程序。
这使得使用 TeAI 编写脚本和自动化任务变得很容易。

这与[CLI 模式](cli-mode)不同,后者是交互式的,更适合主动开发。

## 使用 Python

要在 Python 中以无头模式运行 TeAI,
[请按照开发设置说明](https://github.com/All-Hands-AI/TeAI/blob/main/Development.md),
然后运行:
...
你需要确保通过环境变量
[或 `config.toml` 文件](https://github.com/All-Hands-AI/TeAI/blob/main/config.template.toml)
设置你的模型、API 密钥和其他设置。
