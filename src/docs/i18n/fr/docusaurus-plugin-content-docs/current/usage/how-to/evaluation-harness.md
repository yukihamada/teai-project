...
Ce guide fournit un aperçu de la façon d'intégrer votre propre benchmark d'évaluation dans le framework TeAI.

## Configuration de l'environnement et de la configuration LLM

Veuillez suivre les instructions [ici](https://github.com/All-Hands-AI/TeAI/blob/main/Development.md) pour configurer votre environnement de développement local.
TeAI en mode développement utilise `config.toml` pour garder une trace de la plupart des configurations.
...
## Comment utiliser TeAI en ligne de commande

TeAI peut être exécuté depuis la ligne de commande en utilisant le format suivant :
...
Cette commande exécute TeAI avec :
...
## Comment fonctionne TeAI

Le point d'entrée principal d'TeAI se trouve dans `teai/core/main.py`. Voici un flux simplifié de son fonctionnement :
...
La fonction `run_controller()` est le cœur de l'exécution d'TeAI. Elle gère l'interaction entre l'agent, l'environnement d'exécution et la tâche, en gérant des choses comme la simulation d'entrée utilisateur et le traitement des événements.


## Le moyen le plus simple de commencer : Explorer les benchmarks existants

Nous vous encourageons à examiner les différents benchmarks d'évaluation disponibles dans le [répertoire `evaluation/benchmarks/`](https://github.com/All-Hands-AI/TeAI/blob/main/evaluation/benchmarks) de notre dépôt.
...
1. Importez les utilitaires TeAI pertinents :
   ```python
...
En suivant cette structure, vous pouvez créer un workflow d'évaluation robuste pour votre benchmark dans le framework TeAI.


## Comprendre la `user_response_fn`

La `user_response_fn` est un composant crucial dans le workflow d'évaluation d'TeAI. Elle simule l'interaction de l'utilisateur avec l'agent, permettant des réponses automatisées pendant le processus d'évaluation. Cette fonction est particulièrement utile lorsque vous voulez fournir des réponses cohérentes et prédéfinies aux requêtes ou actions de l'agent.
