...
The default TeAI sandbox (`python-nodejs:python3.12-nodejs22`
...
For example, if you want TeAI to have `ruby` installed, you could create a `Dockerfile` with the following content:
...
When running TeAI using [the docker command](/modules/usage/installation#start-the-app), replace
...
First, ensure you can run TeAI by following the instructions in [Development.md](https://github.com/All-Hands-AI/TeAI/blob/main/Development.md).

### Specify the Base Sandbox Image

In the `config.toml` file within the TeAI directory, set the `base_container_image` to the image you want to use.
...
Run TeAI by running ```make run``` in the top level directory.
