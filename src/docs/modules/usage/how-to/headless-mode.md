...
You can run TeAI with a single command, without starting the web application.
This makes it easy to write scripts and automate tasks with TeAI.

This is different from [CLI Mode](cli-mode), which is interactive, and better for active development.

## With Python

To run TeAI in headless mode with Python:
...
To run TeAI in Headless mode with Docker:
...
docker run -it \
    ...
    python -m teai.core.main -t "write a bash script that prints hi"
```
