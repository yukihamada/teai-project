...
TeAI can connect to any LLM supported by LiteLLM. However, it requires a powerful model to work.
...
Based on these findings and community feedback, the following models have been verified to work reasonably well with TeAI:
...
:::warning
TeAI will issue many prompts to the LLM you configure. Most of these LLMs cost money, so be sure to set spending
limits and monitor usage.
:::

If you have successfully run TeAI with specific LLMs not in the list, please add them to the verified list. We
...
wait times between messages, poor responses, or errors about malformed JSON. TeAI can only be as powerful as the
models driving it. However, if you do find ones that work, please add them to the verified list above.
:::

## LLM Configuration

The following can be set in the TeAI UI through the Settings:
...
We have a few guides for running TeAI with specific model providers:
...
LLM providers typically have rate limits, sometimes very low, and may require retries. TeAI will automatically
...
If you are running TeAI in development mode, you can also set these options in the `config.toml` file:
