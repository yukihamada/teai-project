...
from teai.events.action import FileEditAction, FileWriteAction


def test_view_file(temp_dir, runtime_cls, run_as_teai):
...
def test_view_directory(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Create test file
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
...
def test_create_file(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_create_file_with_empty_content(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        new_file = os.path.join(config.workspace_mount_path_in_sandbox, 'new_file.txt')
        action = FileEditAction(
...
def test_create_with_none_file_text(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_str_replace(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Create test file
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
...
def test_str_replace_multi_line(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_multi_line_with_tabs(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_nonexistent_string(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_with_empty_new_str(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_with_empty_old_str(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_with_none_old_str(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_insert(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Create test file
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
...
def test_insert_invalid_line(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_insert_with_empty_string(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_insert_with_none_new_str(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_undo_edit(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Create test file
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
...
def test_validate_path_invalid(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_create_existing_file_error(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_missing_old_str(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_str_replace_new_str_and_old_str_same(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_insert_missing_line_param(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        test_file = os.path.join(config.workspace_mount_path_in_sandbox, 'test.txt')
        action = FileWriteAction(
...
def test_undo_edit_no_history_error(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_view_large_file_with_truncation(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
