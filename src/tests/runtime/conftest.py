...
from teai.core.config import AppConfig, load_app_config
...
from teai.runtime.plugins import AgentSkillsRequirement, JupyterRequirement
...
RUN_AS_TEAI = os.getenv('RUN_AS_TEAI', 'True').lower() in ['true', '1', 'yes']
test_mount_path = ''
...
def get_run_as_teai() -> list[bool]:
    print(
        '\n\n########################################################################'
    )
    print('USER: ' + 'teai' if RUN_AS_TEAI else 'root')
    print(
        '########################################################################\n\n'
    )
    return [RUN_AS_TEAI]
...
@pytest.fixture(scope='module', params=get_run_as_teai())
def run_as_teai(request):
    time.sleep(1)
    return request.param
...
def _load_runtime(
    ...
    config.run_as_teai = run_as_teai
