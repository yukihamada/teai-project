...
def test_simple_cmd_ipython_and_fileop(temp_dir, runtime_cls, run_as_teai):
    ...
    assert obs.content.strip() == (
        ...
        '[Jupyter Python interpreter: /teai/poetry/teai-ai-5O4_aCHf-py3.12/bin/python]'
    )
...
def test_ipython_multi_user(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    ...
    assert obs.exit_code == 0
    if run_as_teai:
        # -rw-r--r-- 1 teai root 13 Jul 28 03:53 test.txt
        assert 'teai' in obs.content.split('\r\n')[0]
    else:
...
def test_ipython_package_install(temp_dir, runtime_cls, run_as_teai):
...
def test_ipython_file_editor_permissions_as_teai(temp_dir, runtime_cls):
    """Test file editor permission behavior when running as different users."""
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai=True)
    ...
    # Try to view the file as teai user - should fail with permission denied
    ...
    # Try to edit the file as teai user - should fail with permission denied
    ...
    # Try to use file editor in teai sandbox directory - should work
    test_code = """
# Create file
