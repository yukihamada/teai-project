...
from teai.microagent import KnowledgeMicroAgent, RepoMicroAgent, TaskMicroAgent


def _create_test_microagents(test_dir: str):
    """Create test microagent files in the given directory."""
    microagents_dir = Path(test_dir) / '.teai' / 'microagents'
...
"""
    (Path(test_dir) / '.teai_instructions').write_text(legacy_instructions)
...
def test_load_microagents_with_trailing_slashes(
    ...
    try:
        ...
        assert len(repo_agents) == 2  # repo.md + .teai_instructions
...
def test_load_microagents_with_selected_repo(temp_dir, runtime_cls, run_as_teai):
    """Test loading microagents from a selected repository."""
    # Create test files in a repository-like structure
    repo_dir = Path(temp_dir) / 'TeAI'
...
def test_load_microagents_with_missing_files(temp_dir, runtime_cls, run_as_teai):
    """Test loading microagents when some files are missing."""
    # Create only repo.md, no other files
    microagents_dir = Path(temp_dir) / '.teai' / 'microagents'
    microagents_dir.mkdir(parents=True, exist_ok=True)
