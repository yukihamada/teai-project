...
def test_bash_command_env(temp_dir, runtime_cls, run_as_teai):
...
def test_bash_server(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_multiple_multiline_commands(temp_dir, runtime_cls, run_as_teai):
    cmds = [
        'ls -l',
        'echo -e "hello\nworld"',
...
def test_no_ps2_in_output(temp_dir, runtime_cls, run_as_teai):
...
def test_multiline_command_loop(temp_dir, runtime_cls):
    # https://github.com/All-Hands-AI/TeAI/issues/3143
...
def test_cmd_run(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        obs = _run_cmd_action(
        ...
        assert obs.exit_code == 0
        if run_as_teai:
            assert 'teai' in obs.content
        elif runtime_cls == LocalRuntime:
            assert 'root' not in obs.content and 'teai' not in obs.content
        else:
...
def test_run_as_user_correct_home_dir(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        ...
        if runtime_cls == LocalRuntime:
            ...
            assert '/home/teai' in obs.content
        else:
            assert '/root' in obs.content
    finally:
        _close_test_runtime(runtime)
...
def test_git_operation(temp_dir, runtime_cls):
    ...
    try:
        if runtime_cls != LocalRuntime:
            obs = _run_cmd_action(runtime, 'sudo chown -R teai:root .')
            assert obs.exit_code == 0

        # check the ownership of the current directory
        obs = _run_cmd_action(runtime, 'ls -alh .')
        assert obs.exit_code == 0
        # drwx--S--- 2 teai root   64 Aug  7 23:32 .
        ...
        for line in obs.content.split('\n'):
            ...
            if ' ..' in line:
                ...
                assert 'root' in line
                assert 'teai' not in line
            elif ' .' in line:
                # current directory should be owned by teai
        ...
        if runtime_cls == LocalRuntime:
            ...
            obs = _run_cmd_action(
                runtime,
                'git config --file ./.git_config user.name "teai" && git config --file ./.git_config user.email "teai@all-hands.dev"',
            )
            assert obs.exit_code == 0
...
def test_python_version(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_pwd_property(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_basic_command(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Test simple command
...
def test_interactive_command(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(
        temp_dir,
        runtime_cls,
        run_as_teai,
...
def test_long_output(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_long_output_exceed_history_limit(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # Generate a long output
...
def test_long_output_from_nested_directories(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_command_backslash(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_command_output_continuation(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_long_running_command_follow_by_execute(
    temp_dir, runtime_cls, run_as_teai
):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_empty_command_errors(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_python_interactive_input(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
...
def test_stress_long_output_with_soft_and_hard_timeout(
    ...
    try:
        ...
        for i in range(10):
            ...
            mem_action = CmdRunAction(
                'ps aux | awk \'{printf "%8.1f KB  %s\\n", $6, $0}\' | sort -nr | grep "action_execution_server" | grep "/teai/poetry" | grep -v grep | awk \'{print $1}\''
            )
            mem_obs = runtime.run_action(mem_action)
            assert mem_obs.exit_code == 0
            logger.info(
...
def test_bash_remove_prefix(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        # create a git repo
        action = CmdRunAction(
            'git init && git remote add origin https://github.com/All-Hands-AI/TeAI'
        )
        obs = runtime.run_action(action)
        # logger.info(obs, extra={'msg_type': 'OBSERVATION'})
        assert obs.metadata.exit_code == 0
        ...
        # logger.info(obs, extra={'msg_type': 'OBSERVATION'})
        assert obs.metadata.exit_code == 0
        assert 'https://github.com/All-Hands-AI/TeAI' in obs.content
