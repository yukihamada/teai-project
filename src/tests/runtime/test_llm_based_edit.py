...
from teai.events.action import FileEditAction, FileReadAction
from teai.events.observation import FileEditObservation
...
def test_edit_from_scratch(temp_dir, runtime_cls, run_as_teai):
...
def test_edit(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        action = FileEditAction(
            content=ORGINAL,
...
def test_edit_long_file(temp_dir, runtime_cls, run_as_teai):
    runtime, config = _load_runtime(temp_dir, runtime_cls, run_as_teai)
    try:
        action = FileEditAction(
