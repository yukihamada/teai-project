...
from teai.integrations.provider import ProviderType, SecretStore
from teai.server.app import app
from teai.server.settings import Settings


@pytest.fixture
def mock_settings_store():
    with patch('teai.server.routes.settings.SettingsStoreImpl') as mock:
...
def mock_get_user_id():
    with patch('teai.server.routes.settings.get_user_id') as mock:
...
def mock_validate_provider_token():
    with patch('teai.server.routes.settings.validate_provider_token') as mock:
...
def mock_github_service():
    with patch('teai.server.routes.settings.GitHubService') as mock:
        yield mock
...
async def test_settings_api_runtime_factor(
    ...
    with patch('teai.server.shared.config') as mock_config:
