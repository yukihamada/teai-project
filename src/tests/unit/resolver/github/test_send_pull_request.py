...
from teai.core.config import LLMConfig
from teai.resolver.interfaces.github import GithubIssueHandler
from teai.resolver.interfaces.issue import ReviewThread
from teai.resolver.resolver_output import Issue, ResolverOutput
from teai.resolver.send_pull_request import (
    apply_patch,
    initialize_repo,
...
from teai.resolver.utils import Platform
...
@patch('teai.resolver.interfaces.github.GithubIssueHandler.reply_to_comment')
@patch('requests.post')
@patch('subprocess.run')
@patch('teai.resolver.send_pull_request.LLM')
...
def test_send_pull_request(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'teai-fix-issue-42'],
        capture_output=True,
        text=True,
    )
    ...
    if pr_type == 'branch':
        assert (
            result
            == 'https://github.com/test-owner/test-repo/compare/teai-fix-issue-42?expand=1'
        )
        ...
        assert post_data['head'] == 'teai-fix-issue-42'
...
def test_send_pull_request_target_branch_with_fork(
    ...
    assert (
        post_data['head'] == 'teai-fix-issue-42'
...
@patch('teai.resolver.send_pull_request.initialize_repo')
@patch('teai.resolver.send_pull_request.apply_patch')
@patch('teai.resolver.send_pull_request.update_existing_pull_request')
@patch('teai.resolver.send_pull_request.make_commit')
...
@patch('teai.resolver.send_pull_request.initialize_repo')
@patch('teai.resolver.send_pull_request.apply_patch')
@patch('teai.resolver.send_pull_request.send_pull_request')
@patch('teai.resolver.send_pull_request.make_commit')
...
@patch('teai.resolver.send_pull_request.load_all_resolver_outputs')
@patch('teai.resolver.send_pull_request.process_single_issue')
...
def test_send_pull_request_branch_naming(
    ...
    assert checkout_call == call(
        ['git', '-C', repo_path, 'checkout', '-b', 'teai-fix-issue-42-try3'],
        capture_output=True,
        text=True,
    )
    assert push_call == call(
    ...
    assert (
        result
        == 'https://github.com/test-owner/test-repo/compare/teai-fix-issue-42-try3?expand=1'
    )


@patch('teai.resolver.send_pull_request.argparse.ArgumentParser')
@patch('teai.resolver.send_pull_request.process_all_successful_issues')
@patch('teai.resolver.send_pull_request.process_single_issue')
@patch('teai.resolver.send_pull_request.load_single_resolver_output')
@patch('teai.resolver.send_pull_request.identify_token')
...
def test_main(
    ...
    from teai.resolver.send_pull_request import main
