...
from teai.llm.llm import LLM
from teai.resolver.interfaces.gitlab import GitlabIssueHandler, GitlabPRHandler
from teai.resolver.interfaces.issue import Issue, ReviewThread
from teai.resolver.interfaces.issue_definitions import (
    ServiceContextIssue,
    ServiceContextPR,
)
from teai.resolver.resolve_issue import (
    complete_runtime,
    initialize_runtime,
    process_issue,
)
from teai.resolver.resolver_output import ResolverOutput
...
async def test_resolve_issue_no_issues_found():
    from teai.resolver.resolve_issue import resolve_issue
    ...
    with patch(
        'teai.resolver.resolve_issue.issue_handler_factory',
...
async def test_process_issue(mock_output_dir, mock_prompt_template):
    ...
    for test_case in test_cases:
        ...
        with (
            patch(
                'teai.resolver.resolve_issue.create_runtime', mock_create_runtime
            ),
            patch(
                'teai.resolver.resolve_issue.initialize_runtime',
                mock_initialize_runtime,
            ),
            patch(
                'teai.resolver.resolve_issue.run_controller', mock_run_controller
            ),
            patch(
                'teai.resolver.resolve_issue.complete_runtime',
                mock_complete_runtime,
            ),
            patch('teai.resolver.resolve_issue.logger'),
        ):
            # Call the function
...
def test_file_instruction():
    ...
    # load prompt from teai/resolver/prompts/resolve/basic.jinja
    with open('teai/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()
...
def test_file_instruction_with_repo_instruction():
    ...
    with open('teai/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()
    # load repo instruction from teai/resolver/prompts/repo_instructions/all-hands-ai___teai-resolver.txt
    with open(
        'teai/resolver/prompts/repo_instructions/all-hands-ai___teai-resolver.txt',
        'r',
    ) as f:
...
This is a Python repo for teai-resolver, a library that attempts to resolve github issues with the AI agent TeAI.
