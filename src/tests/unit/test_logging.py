...
from teai.core.logger import TeAILoggerAdapter, json_log_handler
from teai.core.logger import teai_logger as teai_logger
...
def test_handler():
    ...
    teai_logger.addHandler(handler)
    yield teai_logger, stream
    teai_logger.removeHandler(handler)
...
def json_handler():
    ...
    teai_logger.addHandler(json_handler)
    yield teai_logger, stream
    teai_logger.removeHandler(json_handler)
...
def test_sensitive_env_vars_masking(test_handler):
    ...
    with patch.dict('teai.core.logger.os.environ', environ, clear=True):
...
class TestJsonOutput:
    ...
    def test_extra_fields_from_adapter(self, json_handler):
        logger, string_io = json_handler
        subject = TeAILoggerAdapter(logger, extra={'context_field': '..val..'})
    ...
    def test_extra_fields_from_adapter_can_override(self, json_handler):
        logger, string_io = json_handler
        subject = TeAILoggerAdapter(logger, extra={'override': 'a'})
