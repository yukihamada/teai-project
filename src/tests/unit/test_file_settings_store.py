...
from teai.storage.settings.file_settings_store import FileSettingsStore
...
async def test_load_nonexistent_data(file_settings_store):
    with patch(
        'teai.server.settings.load_app_config',
...
async def test_get_instance():
    ...
    with patch(
        'teai.storage.settings.file_settings_store.get_file_store'
    ) as mock_get_store:
