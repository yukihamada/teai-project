...
from teai.core.cli import main
...
@pytest.fixture
def mock_runtime():
    with patch('teai.core.cli.create_runtime') as mock_create_runtime:
...
@pytest.fixture
def mock_agent():
    with patch('teai.core.cli.create_agent') as mock_create_agent:
...
@pytest.fixture
def mock_controller():
    with patch('teai.core.cli.create_controller') as mock_create_controller:
...
def mock_config(task_file: Path):
    with patch('teai.core.cli.parse_arguments') as mock_parse_args:
        # Create a proper Namespace with our temporary task file
        args = Namespace(file=str(task_file), task=None, directory=None)
        mock_parse_args.return_value = args
        with patch('teai.core.cli.setup_config_from_args') as mock_setup_config:
