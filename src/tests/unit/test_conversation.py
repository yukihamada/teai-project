...
from teai.runtime.impl.docker.docker_runtime import DockerRuntime
from teai.server.data_models.conversation_info import ConversationInfo
from teai.server.data_models.conversation_info_result_set import (
...
from teai.storage.data_models.conversation_status import ConversationStatus
from teai.storage.locations import get_conversation_metadata_filename
...
def _patch_store():
    ...
    with patch(
        'teai.storage.conversation.file_conversation_store.get_file_store',
        MagicMock(return_value=file_store),
    ):
        with patch(
            'teai.server.routes.manage_conversations.conversation_manager.file_store',
            file_store,
        ):
            yield
...
async def test_search_conversations():
    with _patch_store():
        with patch(
            'teai.server.routes.manage_conversations.config'
        ) as mock_config:
            mock_config.conversation_max_age_seconds = 864000  # 10 days
            with patch(
                'teai.server.routes.manage_conversations.conversation_manager'
            ) as mock_manager:
                ...
                with patch(
                    'teai.server.routes.manage_conversations.datetime'
                ) as mock_datetime:
