...
from teai.runtime.builder.docker import DockerRuntimeBuilder
from teai.runtime.utils.runtime_build import (
    BuildFromImageType,
...
def _check_source_code_in_dir(temp_dir):
    ...
    # The source code should only include the `teai` folder,
    ...
    assert os.path.exists(os.path.join(code_dir, 'teai'))
    assert os.path.isdir(os.path.join(code_dir, 'teai'))
    ...
    assert _pyproject_version == version('teai-ai')
...
def test_generate_dockerfile_build_from_scratch():
    ...
    assert 'COPY ./code/teai /teai/code/teai' in dockerfile_content
    assert (
        '/teai/micromamba/bin/micromamba run -n teai poetry install'
...
def live_docker_image():
    ...
    RUN echo "Hello, TeAI!" > /teai/hello.txt
    """
