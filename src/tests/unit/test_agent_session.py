...
from teai.core.config import AppConfig, LLMConfig
from teai.core.config.agent_config import AgentConfig
from teai.events import EventStream, EventStreamSubscriber
...
async def test_agent_session_start_with_no_state(mock_agent):
    ...
    with patch(
        'teai.server.session.agent_session.AgentController', SpyAgentController
    ), patch(
        'teai.server.session.agent_session.EventStream',
        return_value=mock_event_stream,
    ), patch(
        'teai.controller.state.state.State.restore_from_session',
        side_effect=Exception('No state found'),
    ), patch('teai.server.session.agent_session.Memory', return_value=memory):
        await session.start(
...
async def test_agent_session_start_with_restored_state(mock_agent):
    ...
    ), patch('teai.server.session.agent_session.Memory', mock_memory):
        await session.start(
            runtime_name='test-runtime',
            config=AppConfig(),
            agent=mock_agent,
