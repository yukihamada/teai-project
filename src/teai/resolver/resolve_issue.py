...
from teai.core.config import AgentConfig, AppConfig, LLMConfig, SandboxConfig
from teai.core.logger import teai_logger as logger
from teai.core.main import create_runtime, run_controller
from teai.events.action import CmdRunAction, MessageAction
...
from teai.events.stream import EventStreamSubscriber
...
async def resolve_issue(
    ...
    if repo_instruction is None:
        # Check for .teai_instructions file in the workspace directory
        teai_instructions_path = os.path.join(repo_dir, '.teai_instructions')
        if os.path.exists(teai_instructions_path):
            with open(teai_instructions_path, 'r') as f:
...
def main() -> None:
    ...
    if runtime_container_image is None and not my_args.is_experimental:
        runtime_container_image = (
            f'ghcr.io/all-hands-ai/runtime:{teai.__version__}-nikolaik'
        )
