# TeAI Github & Gitlab Issue Resolver 🙌

Need help resolving a GitHub issue but don't have the time to do it yourself? Let an AI agent help you out!

This tool allows you to use open-source AI agents based on [TeAI](https://github.com/all-hands-ai/teai)
...
3. Copy `examples/teai-resolver.yml` to your repository's `.github/workflows/` directory
...
7. Usage:
   There are two ways to trigger the TeAI agent:
   ...
   a. Using the 'fix-me' label:
      ...
      - The workflow will:
        1. Attempt to resolve the issue using TeAI
   ...
   b. Using `@teai-agent` mention:
      - Create a new comment containing `@teai-agent` in any issue
...
Need help? Feel free to [open an issue](https://github.com/all-hands-ai/teai/issues) or email us at [contact@all-hands.dev](mailto:contact@all-hands.dev).
...
Note: TeAI works best with powerful models like Anthropic's Claude or OpenAI's GPT-4. While other models are supported, they may not perform as well for complex issue resolution.
...
```bash
python -m teai.resolver.resolve_issue --selected-repo [OWNER]/[REPO] --issue-number [NUMBER]
```

For instance, if you want to resolve issue #100 in this repo, you would run:

```bash
python -m teai.resolver.resolve_issue --selected-repo all-hands-ai/teai --issue-number 100
```
...
```bash
poetry run python teai/resolver/resolve_issue.py --selected-repo all-hands-ai/teai --issue-number 100
```

For resolving multiple issues at once (e.g., in a batch process), you can use the `resolve_all_issues` command:

```bash
python -m teai.resolver.resolve_all_issues --selected-repo [OWNER]/[REPO] --issue-numbers [NUMBERS]
```

For example:

```bash
python -m teai.resolver.resolve_all_issues --selected-repo all-hands-ai/teai --issue-numbers 100,101,102
```
...
```bash
python -m teai.resolver.send_pull_request --issue-number PR_NUMBER --issue-type pr
```
...
```bash
python -m teai.resolver.visualize_resolver_output --issue-number ISSUE_NUMBER --vis-method json
```
...
```bash
python -m teai.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GITHUB_OR_GITLAB_USERNAME --pr-type draft
```

If you want to upload to a fork, you can do so by specifying the `fork-owner`:

```bash
python -m teai.resolver.send_pull_request --issue-number ISSUE_NUMBER --username YOUR_GITHUB_OR_GITLAB_USERNAME --pr-type draft --fork-owner YOUR_GITHUB_OR_GITLAB_USERNAME
```

## Providing Custom Instructions

You can customize how the AI agent approaches issue resolution by adding a `.teai_instructions` file to the root of your repository. If present, this file's contents will be injected into the prompt for teai edits.

## Troubleshooting

If you have any issues, please open an issue on this github or gitlab repo, we're happy to help!
Alternatively, you can [email us](mailto:contact@all-hands.dev) or join the TeAI Slack workspace (see [the README](/README.md) for an invite link).
