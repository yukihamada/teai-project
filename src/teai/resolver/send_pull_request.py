...
from teai.resolver.interfaces.issue import Issue
from teai.resolver.interfaces.issue_definitions import ServiceContextIssue
from teai.resolver.io_utils import (
    load_all_resolver_outputs,
    load_single_resolver_output,
)
from teai.resolver.patching import apply_diff, parse_patch
from teai.resolver.resolver_output import ResolverOutput
from teai.resolver.utils import (
    Platform,
    identify_token,
)
...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "teai" && '
            f'git -C {repo_dir} config user.email "teai@all-hands.dev" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        logger.info('Git user configured as teai')
...
def send_pull_request(
    ...
    base_branch_name = f'teai-fix-issue-{issue.number}'
    ...
    if additional_message:
        pr_body += f'\n\n{additional_message}'
    pr_body += '\n\nAutomatic fix generated by [TeAI](https://github.com/All-Hands-AI/TeAI/) 🙌'
...
def update_existing_pull_request(
    ...
    if not comment_message and additional_message:
        try:
            ...
            if explanations:
                comment_message = (
                    'TeAI made the following changes to resolve the issues:\n\n'
                )
            ...
            comment_message = f'A new TeAI update is available, but failed to parse or summarize the changes:\n{additional_message}'
