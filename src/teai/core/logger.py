...
def get_file_handler(
    ...
    file_name = f'teai_{timestamp}.log'
...
teai_logger = logging.getLogger('teai')
current_log_level = logging.INFO

if LOG_LEVEL in logging.getLevelNamesMapping():
    current_log_level = logging.getLevelNamesMapping()[LOG_LEVEL]
teai_logger.setLevel(current_log_level)

if DEBUG:
    teai_logger.addFilter(StackInfoFilter())

if current_log_level == logging.DEBUG:
    LOG_TO_FILE = True
    teai_logger.debug('DEBUG mode enabled.')

if LOG_JSON:
    teai_logger.addHandler(json_log_handler(current_log_level))
else:
    teai_logger.addHandler(get_console_handler(current_log_level))

teai_logger.addFilter(SensitiveDataFilter(teai_logger.name))
teai_logger.propagate = False
teai_logger.debug('Logging initialized')

LOG_DIR = os.path.join(
    # parent dir of teai/core (i.e., root of the repo)
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    'logs',
)

if LOG_TO_FILE:
    teai_logger.addHandler(
        get_file_handler(LOG_DIR, current_log_level)
    )  # default log to project root
    teai_logger.debug(f'Logging to file in: {LOG_DIR}')
...
class LlmFileHandler(logging.FileHandler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        super().emit(record)
        self.stream.close()
        teai_logger.debug('Logging to %s', self.baseFilename)
        self.message_counter += 1
...
class TeAILoggerAdapter(logging.LoggerAdapter):
    extra: dict

    def __init__(self, logger=teai_logger, extra=None):
        self.logger = logger
        self.extra = extra or {}
