...
from teai.events.event import Event, EventSource, RecallType
from teai.events.observation.agent import (
    MicroagentKnowledge,
    RecallObservation,
)
from teai.events.observation.empty import NullObservation
from teai.events.stream import EventStream, EventStreamSubscriber
...
from teai.utils.prompt import RepositoryInfo, RuntimeInfo

GLOBAL_MICROAGENTS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(teai.__file__)),
    'microagents',
)
...
class Memory:
    ...
    def __init__(
        ...
        # from typically TeAI/microagents (i.e., the PUBLIC microagents)
