...
The TeAI Runtime folder contains the core components responsible for executing actions and managing the runtime environment for the TeAI project. This README provides an overview of the main components and their interactions.
...
6. **Sandbox Environment**:
   ...
   - Actions received from the TeAI backend are executed in this sandboxed environment.
...
This is the default runtime used within TeAI.
...
**Important: This runtime provides no isolation as it runs directly on the host machine. All actions are executed with the same permissions as the user running TeAI. For secure execution with proper isolation, use the Docker Runtime instead.**
...
At the time of this writing, this is mostly used in parallel evaluation, such as this example for [SWE-Bench](https://github.com/All-Hands-AI/TeAI/tree/main/evaluation/benchmarks/swe_bench#run-inference-on-remoteruntime-experimental).

## Related Components

- The runtime interacts closely with the event system defined in the `teai.events` module.
- It relies on configuration classes from `teai.core.config`.
- Logging is handled through `teai.core.logger`.

This section provides an overview of the TeAI Runtime folder. For more detailed information on specific components or usage, please refer to the individual files and their docstrings.
