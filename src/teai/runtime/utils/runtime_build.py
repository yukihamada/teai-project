...
from teai.runtime.builder import DockerRuntimeBuilder, RuntimeBuilder
...
def build_runtime_image(
    ...
    If dry_run is False, it will also build the TeAI runtime Docker image using the docker build folder.
...
def prep_build_folder(
    ...
    teai_source_dir = Path(teai.__file__).parent
    project_root = teai_source_dir.parent
    logger.debug(f'Building source distribution using project root: {project_root}')

    # Copy the 'teai' directory (Source code)
    shutil.copytree(
        teai_source_dir,
        Path(build_folder, 'code', 'teai'),
...
def get_hash_for_lock_files(base_image: str) -> str:
    ...
    for file in ['pyproject.toml', 'poetry.lock']:
        src = Path(teai_source_dir, file)
        if not src.exists():
            src = Path(teai_source_dir.parent, file)
