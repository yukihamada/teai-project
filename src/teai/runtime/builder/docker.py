...
from teai import __version__ as oh_version
from teai.core.exceptions import AgentRuntimeBuildError
from teai.core.logger import RollingLogger
from teai.core.logger import teai_logger as logger
from teai.runtime.builder.base import RuntimeBuilder
from teai.utils.term_color import TermColor, colorize
...
class DockerRuntimeBuilder(RuntimeBuilder):
    ...
    def build(
        ...
        if not DockerRuntimeBuilder.check_buildx(self.is_podman):
            # when running teai in a container, there might not be a "docker"
            # binary available, in which case we need to download docker binary.
            # since the official teai app image is built from debian, we use
            # debian way to install docker binary
            logger.info(
                'No docker binary available inside teai-app container, trying to download online...'
            )
            commands = [
                'apt-get update',
        ...
        buildx_cmd = [
            ...
            'build',
            '--progress=plain',
            f'--build-arg=TEAI_RUNTIME_VERSION={oh_version}',
            f'--build-arg=TEAI_RUNTIME_BUILD_TIME={datetime.datetime.now().isoformat()}',
