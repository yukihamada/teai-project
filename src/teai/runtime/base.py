...
from teai.core.config import AppConfig, SandboxConfig
from teai.core.exceptions import AgentRuntimeDisconnectedError
from teai.core.logger import teai_logger as logger
from teai.events import EventSource, EventStream, EventStreamSubscriber
...
from teai.events.serialization.action import ACTION_TYPE_TO_CLASS
from teai.integrations.provider import (
    PROVIDER_TOKEN_TYPE,
    ProviderHandler,
    ProviderType,
)
from teai.microagent import (
    BaseMicroAgent,
    load_microagents_from_dir,
)
from teai.runtime.plugins import (
    JupyterRequirement,
    PluginRequirement,
    VSCodeRequirement,
)
from teai.runtime.utils.edit import FileEditRuntimeMixin
from teai.utils.async_utils import (
    GENERAL_TIMEOUT,
...
class Runtime(FileEditRuntimeMixin):
    ...
    def clone_repo(
        ...
        teai_workspace_branch = f'teai-workspace-{random_str}'
        ...
        checkout_command = (
            ...
            if selected_branch
            else f'git checkout -b {teai_workspace_branch}'
        )
    ...
    def maybe_run_setup_script(self):
        """Run .teai/setup.sh if it exists in the workspace or repository."""
        setup_script = '.teai/setup.sh'
    ...
    def get_microagents_from_selected_repo(
        ...
        microagents_dir = workspace_root / '.teai' / 'microagents'
        repo_root = None
        if selected_repository:
            repo_root = workspace_root / selected_repository.split('/')[-1]
            microagents_dir = repo_root / '.teai' / 'microagents'
        self.log(
            'info',
        ...
        # Check for legacy .teai_instructions file
        obs = self.read(
            FileReadAction(path=str(workspace_root / '.teai_instructions'))
        )
        if isinstance(obs, ErrorObservation) and repo_root is not None:
            ...
            self.log(
                'debug',
                f'.teai_instructions not present, trying to load from repository {microagents_dir=}',
            )
            obs = self.read(
                FileReadAction(path=str(repo_root / '.teai_instructions'))
            )

        if isinstance(obs, FileReadObservation):
            self.log('info', 'teai_instructions microagent loaded.')
            loaded_microagents.append(
                BaseMicroAgent.load(
                    path='.teai_instructions', file_content=obs.content
                )
            )
