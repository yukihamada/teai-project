...
from teai.events import EventStream
...
from teai.events.serialization import event_to_dict, observation_from_dict
from teai.runtime.impl.action_execution.action_execution_client import (
...
from teai.runtime.utils.command import get_action_execution_server_startup_command
...
def check_dependencies(code_repo_path: str, poetry_venvs_path: str):
    ERROR_MESSAGE = 'Please follow the instructions in https://github.com/All-Hands-AI/TeAI/blob/main/Development.md to install TeAI.'
...
class LocalRuntime(ActionExecutionClient):
    ...
    def __init__(
        ...
        if self.config.workspace_base is not None:
            ...
            self._temp_workspace = tempfile.mkdtemp(
                prefix=f'teai_workspace_{sid}',
            )
            self.config.workspace_mount_path_in_sandbox = self._temp_workspace

        logger.warning(
            'Initializing LocalRuntime. WARNING: NO SANDBOX IS USED. '
            'This is an experimental feature, please report issues to https://github.com/All-Hands-AI/TeAI/issues. '
            '`run_as_teai` will be ignored since the current user will be used to launch the server. '
    ...
    async def connect(self):
        ...
        code_repo_path = os.path.dirname(os.path.dirname(teai.__file__))
        env['PYTHONPATH'] = f'{code_repo_path}:$PYTHONPATH'
        env['TEAI_REPO_PATH'] = code_repo_path
