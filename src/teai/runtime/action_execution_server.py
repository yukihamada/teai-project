...
It is responsible for executing actions received from TeAI backend and producing observations.
...
from teai_aci.editor.editor import OHEditor
from teai_aci.editor.exceptions import ToolError
from teai_aci.editor.results import ToolResult
from teai_aci.utils.diff import get_diff
from pydantic import BaseModel
from starlette.background import BackgroundTask
from starlette.exceptions import HTTPException as StarletteHTTPException
from uvicorn import run

from teai.core.exceptions import BrowserUnavailableException
...
from teai.events.event import FileEditSource, FileReadSource
...
from teai.events.serialization import event_from_dict, event_to_dict
from teai.runtime.browser import browse
from teai.runtime.browser.browser_env import BrowserEnv
from teai.runtime.plugins import ALL_PLUGINS, JupyterPlugin, Plugin, VSCodePlugin
from teai.runtime.utils.bash import BashSession
from teai.runtime.utils.files import insert_lines, read_lines
from teai.runtime.utils.memory_monitor import MemoryMonitor
from teai.runtime.utils.runtime_init import init_user_and_working_directory
from teai.runtime.utils.system_stats import get_system_stats
from teai.utils.async_utils import call_sync_from_async, wait_all
...
class ActionExecutor:
    ...
    async def ainit(self):
        ...
        if 'agent_skills' in self.plugins and 'jupyter' in self.plugins:
            obs = await self.run_ipython(
                IPythonRunCellAction(
                    code='from teai.runtime.plugins.agent_skills.agentskills import *\n'
                )
            )
    ...
    async def _init_bash_commands(self):
        INIT_COMMANDS = [
            'git config --file ./.git_config user.name "teai" && git config --file ./.git_config user.email "teai@all-hands.dev" && alias git="git --no-pager" && export GIT_CONFIG=$(pwd)/.git_config'
            if os.environ.get('LOCAL_RUNTIME_MODE') == '1'
            else 'git config --global user.name "teai" && git config --global user.email "teai@all-hands.dev" && alias git="git --no-pager"'
        ]
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--username', type=str, help='User to run as', default='teai'
    )
