...
For example, `teai/agenthub/codeact_agent`, etc.
...
The `state` represents the running state of an agent in the TeAI system. The class handles saving and restoring the agent session. It is serialized in a pickle.
...
TeAI is a multi-agentic system. Agents can delegate tasks to other agents, whether
...
A `task` is an end-to-end conversation between TeAI (the whole system) and the user,
...
TeAI repo has`. Let's assume the default agent is CodeActAgent.
