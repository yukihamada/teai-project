# BioCoder Evaluation with Openteai
...
In the teai branch of the Biocoder repository, we have slightly modified our original Docker image to work with the TeAI environment. In the Docker image are testing scripts (`/testing/start_test_teai.py` and aux files in `/testing_files/`) to assist with evaluation. Additionally, we have installed all dependencies, including OpenJDK, mamba (with Python 3.6), and many system libraries. Notably, we have **not** packaged all repositories into the image, so they are downloaded at runtime.
...
To reproduce this image, please see the Dockerfile_Openteai in the `biocoder` repository.
...
with current TeAI version, then your command would be:
