...
from teai.events.serialization.event import event_to_dict
from teai.runtime.base import Runtime
from teai.utils.async_utils import call_async_from_sync
from teai.utils.shutdown_listener import sleep_if_should_continue
...
# TODO: migrate all swe-bench docker to ghcr.io/teai
...
def get_config(
    ...
    logger.info(
        ...
        f'Submit an issue on https://github.com/All-Hands-AI/TeAI if you run into any issues.'
    )
...
def initialize_runtime(
    ...
    action = CmdRunAction(command='git checkout -b teai')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        obs.exit_code == 0, f'Failed to git checkout new branch teai: {str(obs)}'
    )
...
def complete_runtime(
    ...
    action = CmdRunAction(command='git commit -m "teai edits"')
    action.set_hard_timeout(600)
    logger.info(action, extra={'msg_type': 'ACTION'})
    obs = runtime.run_action(action)
    logger.info(obs, extra={'msg_type': 'OBSERVATION'})
    assert_and_raise(
        ...
        f'Failed to git commit -m "teai": {str(obs)}',
    )
...
if __name__ == '__main__':
    ...
    # so we don't need to manage file uploading to TeAI's repo
    ...
    details = {}
    _agent_cls = teai.agenthub.Agent.get_cls(args.agent_cls)
